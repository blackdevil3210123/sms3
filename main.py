...
# ========== UPDATED: NEW BOT TOKEN & ADMIN ==========
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8282449373:AAE2rbAbecrb7-mGNswk0lWki73xPXytWU8")
...
async def resolve_pending_force_join(bot: Bot):
    """Auto-resolve pending force-join channel IDs using invite links via getChat on @username links.
    For private '+' links, this captures the real numeric ID once the bot is added as admin
    to the channel/group (detected via my_chat_member updates)."""
    d = load()
    fj = d.get("force_join", {})
    ...
    for ch in channels:
        cid = str(ch.get("id", ""))
        if cid.startswith("PENDING_RESOLVE"):
            link = ch.get("link", "")
            # Try resolving via the link (works for public t.me/<username> links)
            if link and not link.startswith("https://t.me/+"):
                uname = link.rsplit("/", 1)[-1].lstrip("@")
                try:
                    chat = await bot.get_chat(f"@{uname}")
                    ch["id"] = str(chat.id)
                    ch["title"] = chat.title or ch.get("title", "Channel")
                    changed = True
                    log.info(f"Resolved force-join channel '{ch['title']}' -> {chat.id}")
                except Exception as e:
                    log.warning(f"Could not resolve {link}: {e}")
    if changed:
        save(d)

async def check_membership(bot: Bot, uid: int, channel_id: str) -> bool:
    cid = str(channel_id).strip()
    # Pending/unresolved channel (private '+' link, bot not yet admin) -> treat as not joined
...
async def cmd_start_deep(msg: Message, state: FSMContext):
    ...
    if not can_use(uid, d):
        await msg.answer(f"{em(EMOJI_CROSS, '⛔')} <b>Access nahi hai!</b>\n\nOwner se approval lein: {SUPER_ADMIN_NAME}", parse_mode="HTML")
        return
...
async def on_my_chat_member(event: ChatMemberUpdated, bot: Bot):
    """When the bot is added to a channel/group as admin, capture its numeric ID
    and auto-map it to the pending force-join entry that shares the same invite link."""
    try:
        chat = event.chat
        ...
        if new_status in ("administrator", "member"):
            ...
            for ch in channels:
                ...
                if cid.startswith("PENDING_RESOLVE"):
                    link = ch.get("link", "")
                    # Match: try to get the chat's invite link and compare, or
                    # if only one pending entry remains, assign automatically.
                    try:
                        chat_invite = await bot.export_chat_invite_link(chat.id)
                    except Exception:
                        chat_invite = ""
                    if (link and chat_invite and link == chat_invite):
                        ch["id"] = str(chat.id)
                        ch["title"] = chat.title or ch.get("title", "Channel")
                        changed = True
                        log.info(f"[my_chat_member] Mapped '{ch['title']}' -> {chat.id}")
            if not changed:
                # Fallback: assign the first remaining pending entry to this chat
                for ch in channels:
                    cid = str(ch.get("id", ""))
                    if cid.startswith("PENDING_RESOLVE"):
                        ch["id"] = str(chat.id)
                        ch["title"] = chat.title or ch.get("title", "Channel")
                        changed = True
                        log.info(f"[my_chat_member] Auto-assigned '{ch['title']}' -> {chat.id}")
                        break
            if changed:
                save(d)
                try:
                    await bot.send_message(
                        MAIN_OWNER,
                        f"{em(EMOJI_CHECK, '✅')} <b>Force-Join channel auto-linked!</b>\n"
                        f"{em(EMOJI_BELL, '📢')} {chat.title}\n"
                        f"{em(EMOJI_GEAR, '🆔')} <code>{chat.id}</code>\n\n"
                        f"Ab bot membership check kar sakta hai.",
                        parse_mode="HTML"
                    )
                except Exception:
                    pass
    except Exception as e:
        log.error(f"my_chat_member handler error: {e}")
...
async def main():
    ...
    # Auto-resolve pending force-join channel IDs (for public links)
    try:
        await resolve_pending_force_join(bot)
    except Exception as e:
        log.warning(f"Force-join resolve error: {e}")

    # Pre-load firebases & scan
    d = load()
    log.info(f"Loaded config: {len(d.get('firebases', []))} firebases, force_join={d.get('force_join', {}).get('enabled')}")
    ...
    try:
        await bot.send_message(
            ...
            f"{em(EMOJI_FIRE, '🔥')} <b>Firebases loaded:</b> {len(d.get('firebases', []))}\n"
            f"{em(EMOJI_BELL, '🔗')} <b>Force Join:</b> {'ON' if d.get('force_join', {}).get('enabled') else 'OFF'} ({len(d.get('force_join', {}).get('channels', []))} channels)\n"
            ...
            f"<i>Bot ko dono channels/groups mein admin banake add karein — IDs auto-capture ho jayenge.</i>",
            parse_mode="HTML"
        )
    except Exception as e:
